...
const copyCsv = async (table, file) => {
    ...
    let query = ''
    if (table === 'products') {
        query += `COPY products(product_id, avgRating, totalReviews, totalRatings) FROM '/Users/hannahmanfredi/Desktop/SDC/SDC-CustomerReviews/products.csv' CSV HEADER`;
    } else if (table === 'reviews') {
        query += `COPY reviews(review_id, title, abuseReported, rating, location_, userId, productId, reviewDate, reviewBody, helpfulCount) FROM '/Users/hannahmanfredi/Desktop/SDC/SDC-CustomerReviews/reviews.csv' CSV HEADER`;
    } else if (table === 'users') {
        query += `COPY users(userid, name_, userrating, totalreviews) FROM '/Users/hannahmanfredi/Desktop/SDC/SDC-CustomerReviews/users.csv' CSV HEADER`;
    } else if (table === 'features') {
        query += `COPY features(feature_id, name_) FROM '/Users/hannahmanfredi/Desktop/SDC/SDC-CustomerReviews/features.csv' CSV HEADER`;
    } else if (table === 'productFeatures') {
        query += `COPY productfeatures(productFeature_id, productid, featureid) FROM '/Users/hannahmanfredi/Desktop/SDC/SDC-CustomerReviews/productFeatures.csv' CSV HEADER`;
    }
    return (
      fastcsv
        .parse()
